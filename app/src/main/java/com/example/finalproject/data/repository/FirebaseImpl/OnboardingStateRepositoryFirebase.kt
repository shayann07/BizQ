// app/src/main/java/com/example/finalproject/data/repository/FirebaseImpl/OnboardingStateRepositoryFirebase.kt
...
@Singleton
class OnboardingStateRepositoryFirebase @Inject constructor(
    ...
    override fun observe(uid: String): LiveData<OnboardingState?> = onboardingDao.observe(uid)
    override suspend fun getLocal(uid: String): OnboardingState? = onboardingDao.get(uid)
    override suspend fun upsertLocal(state: OnboardingState) { onboardingDao.upsert(state) }
    ...
    override suspend fun fetchRemote(uid: String): Resource<OnboardingState?> = safeCall {
        ...
        Resource.Success(snap.toObject(OnboardingState::class.java))
    }
